...
module.exports = {
    ...
    aliases : ['tmute'],
    cooldown: 1000,
    description: "Mute users over time",
    async execute  (client, message, args) {
        ...
        if(!time) return message.channel.send(embed.setDescription('<a:error:854851816289075260> | Please specify a time.'))
        ...
        if(!role) {
            try {
                ...
                message.channel.send(embed.setDescription('<a:error:854851816289075260> | Muted role has sucessfully been created.'))
            } catch (error) {
                console.log(error)
            }
        };
